...
modelling is a data analysis activity that can be crowd-sourced.}
Shortcomings \phil{of the implementation itself include} (a)~lens
profiles that tend to be systematically shallower than the lenses
\phil{(leading to systematic over-estimation of the Einstein radius 
by some $\sim25\%$}, and (b)~lack of detailed \phil{arc} modelling. 
Ideas for improvement are discussed. 
...
video-conferencing and in-person meetings where possible. But
preliminary results are immediately summarized on modelling threads on
the forum, and anyone interested is welcome to join at any time.
...
no significance.  Only the heirarchy of which loop is inside which is
...
\item \spl tends to overestimate the Einstein radius (evident from
  Figure \ref{fig:ER_per_sim}), and it is also apparent that the
  models tend to be too shallow.  The likely explanation is that while
  the sims are steeply peaked at the centre, the pixellated mass model
  fixes a comparatively large area near the central at constant
  density.  One way to solve this problem would be to introduce
  smaller pixels in the central region, thus enabled a steeper centre.
