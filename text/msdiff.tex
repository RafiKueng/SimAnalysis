...
%DIF DEL ms-before.tex   Mon Jan 20 14:50:31 2014
%DIF ADD ms.tex          Mon Jan 20 15:01:47 2014
...
\begin{abstract}
In \DIFdelbegin \DIFdel{Spacewarps, volunteers are }\DIFdelend \DIFaddbegin \DIFadd{the initial phase of Spacewarps (SW1), volunteers were }\DIFaddend invited to search \DIFaddbegin \DIFadd{the }\DIFaddend CFHTLS sky survey \DIFdelbegin \DIFdel{for
lens candidates}\DIFdelend \DIFaddbegin \DIFadd{to look for gravitational lenses}\DIFaddend . Here we report on a \DIFdelbegin \DIFdel{system that allows experienced volunteers to model lens candidates .  A }\DIFdelend \DIFaddbegin \DIFadd{web interface    
that gives experienced volunteers the opportunity to model the candidates that have been identified. These volunteers, were also invited to model a }\DIFaddend sample of 29 simulated lenses\DIFdelbegin \DIFdel{were modeled, each multiple times.  The }\DIFdelend \DIFaddbegin \DIFadd{, to order to gauge the }\DIFaddend quality of the models \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{that were being generated. These were }\DIFaddend then examined in \DIFdelbegin \DIFdel{different ways.  In particular }\DIFdelend \DIFaddbegin \DIFadd{greater detail, with particular attention being paid to the following}\DIFaddend : (i)~\DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{identification of }\DIFaddend image parities and time arrivals\DIFdelbegin \DIFdel{were identified correctly in most cases, but not
always}\DIFdelend ; (ii)~the mean convergence (equivalent to the enclosed mass)\DIFdelbegin \DIFdel{was quite well constrained in the image region}\DIFdelend \DIFaddbegin \DIFadd{, and finally}\DIFaddend ;
 (iii)~ the performance of \DIFdelbegin \DIFdel{experienced volunteers was comparable to professionals.
}\DIFdelend \DIFaddbegin \DIFadd{a volunteer vs. a professional. In most cases, the volunteers were found to correctly identify the image parities and time arrivals, along with a mean convergence that was contained within the image region; In all, the results could be comparable to that of professionals.
}

\todo[red]{or should that be web application? Also, I change the tense of the opening statement. Not too sure of the correct way of introducing an abbreviation (SW1 in this case), and I was a bit liberal with my editing - Sorry. Another comment - is 'Well contained' better than 'contained' or is it just a figure of speech?} 
