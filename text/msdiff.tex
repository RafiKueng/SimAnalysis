...
%DIF DEL ms-before.tex   Mon Jan 20 15:48:08 2014
%DIF ADD ms.tex          Mon Jan 20 15:48:08 2014

\usepackage{amsmath}
\usepackage{xspace}     % fixes spaces after custom commands
\usepackage{float}      % exact placement of figures (using \begin{figure}[H])
%DIF 6-8c6
%DIF < \usepackage{color}  % well... color...
%DIF < \usepackage{setspace}    % remove this pgk for final compile!! 
%DIF < \usepackage{subfigure}
%DIF -------
\usepackage{subfigure}  % does what it says %DIF > 
%DIF -------

%DIF 10-11c8-9
%DIF < \usepackage{longtable}   % those two to generate the 2 ol table
%DIF < \usepackage{multicol}
%DIF -------
%\usepackage{longtable}   % those two to generate the 2 ol table %DIF > 
...
%DIF -------
% !!! REMOVE THIS LINES FOR FINAL COMPILE %DIF > 
% %DIF > 
\usepackage{setspace}    % remove this pgk for final compile!!  %DIF > 
\usepackage{color}    % well... color... %DIF > 
%DIF -------

%DIF 52a52
\newcommand{\todo}[2][red]{% %DIF > 
%DIF -------
...
{\em Spacewarps\/} project\footnote{\tt http://www.spacewarps.org}
 \DIFdelbegin \DIFdel{(Saha is a co-investigator) }\DIFdelend has been launched.  {\em Spacewarps\/} is
...
are too difficult for robots and too large for specialists.  In {\em
 Spacewarps\/} itself, \DIFdelbegin \DIFdel{since the launch in }\DIFdelend \DIFaddbegin \DIFadd{survey sky images are presented for volunteers to identify lens candidates.  The first tranche of survey data, introduced from }\DIFaddend May 2013, \DIFdelbegin \DIFdel{$\sim100$ }\DIFdelend \DIFaddbegin \DIFadd{is an area of $\sim172$ }\DIFaddend square
degrees (or a quarter of a percent of the sky) \DIFdelbegin \DIFdel{has been examined}\DIFdelend \DIFaddbegin \DIFadd{from the Canada-France-Hawaii Telescope Legacy Survey}\DIFaddend .
...
\DIFaddbegin \DIFadd{These data had previously been analyzed by robots \mbox{%DIFAUXCMD
\cite{Gavazzi2012, More2012ApJ}
}%DIFAUXCMD
.  
  }\DIFaddend A list of candidates is being
processed, but intermediate results show some very good candidates, as
well \DIFaddbegin \DIFadd{as }\DIFaddend re-discoveries of \DIFdelbegin \DIFdel{a few known lenses}\DIFdelend \DIFaddbegin \DIFadd{candidates previously found robotically}\DIFaddend .
...
\citep{2001astro.ph..2341K,2001astro.ph..2340K} \DIFaddbegin \DIFadd{using the CFHTLS survey data
and catalogues \mbox{%DIFAUXCMD
\cite{Coupon2009}
}%DIFAUXCMD
}\DIFaddend . They were of three
kinds, as follows.
...
\begin{enumerate}
  ...
  \item Resembling cluster lenses: having a source as above, but a
    ...
    the \DIFdelbegin \DIFdel{intergalactic mass}\DIFdelend \DIFaddbegin \DIFadd{underlying dark matter distribution}\DIFaddend .
\end{enumerate}
...
%DIFDELCMD <   \centering
%DIFDELCMD <     \includegraphics[width=0.80\textwidth]{fig/eR_1.png}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \ERf %%%
\DIFdel{for all models with estimated errors in blue squares, }%DIFDELCMD < \ERg %%%
\DIFdel{of simulation in red crosses}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:ER_all_models}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend To compare the enclosed mass profile and the Einstein radius of the simulation and the models, \kenc was calculated using the mass map \kap[x,y] generated in the modeling process.
...
These results can be seen in \DIFdelbegin \DIFdel{\figref{ER_all_models}}\DIFdelend \DIFaddbegin \DIFadd{\figref{ER_per_sim}}\DIFaddend .
...
\begin{figure}
  ...
  \DIFaddbeginFL \subfigure{\includegraphics[width=0.45\textwidth]
             {fig/007024_kappa_encl}}
  \DIFaddendFL \subfigure{\includegraphics[width=0.45\textwidth]
             {fig/007021_kappa_encl}} \\
  \DIFdelbeginFL %DIFDELCMD < \subfigure{\includegraphics[width=0.45\textwidth]
%DIFDELCMD <              {fig/007024_kappa_encl}}
%DIFDELCMD <   \subfigure{\includegraphics[width=0.45\textwidth]
%DIFDELCMD <              {fig/007025_kappa_encl}}
%DIFDELCMD <   \subfigure{\includegraphics[width=0.45\textwidth]
%DIFDELCMD <              {fig/007022_kappa_encl}}
%DIFDELCMD <   %%%
\DIFdelendFL \caption{\DIFaddbeginFL \DIFaddFL{Enclosed mass }\DIFaddendFL \kenc for models of \asw{0h2m} \DIFdelbeginFL \DIFdelFL{: 7020 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{with incorrect image identifications. \figref{7022} shows a model }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{top left}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{by an expert}\DIFaddendFL ) \DIFaddbeginFL \DIFaddFL{with the correct identifications}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{7021 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{while \figref{7025} shows a model with error E06 }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{top right), 7024 (mid left), 7025 (mid right) }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{parities rotated }\DIFaddendFL by \DIFdelbeginFL \DIFdelFL{volunteers }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\pi/2$).  Shown here are the }\kenc \DIFaddFL{results for two models with error E06 }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{a correct }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{one }\DIFaddendFL model \DIFdelbeginFL \DIFdelFL{7022 }\DIFdelendFL (bottom) \DIFaddbeginFL \DIFaddFL{with error E05 (parities rotated }\DIFaddendFL by \DIFdelbeginFL \DIFdelFL{an expert}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\pi$)}\DIFaddendFL .}
...
\DIFdelbegin \DIFdel{E05 and E06 happen mostly in very symmetric images, that are hard to come up with a unique valid solution.  This errors change }\DIFdelend \DIFaddbegin \DIFadd{Comparing the models from volunteers and experts can be done in Figures \ref{fig:7022}, \ref{fig:7025} and \ref{fig:kapenc_compare_faulty}, where only the
expert got the right configuration (\figref{7022}) but enclosed masses are all fairly similar.  In cases with nearly symmetric image configurations, like this example, it is more difficult to identify the image parities correctly.  Incorrect image-parity identification changes }\DIFaddend the orientation of the mass distribution \kap[x,y],
but \kenc and thus the \ERf \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend not influenced that much\DIFdelbegin \DIFdel{, and thus the final result is still a valid model.
This can be seen by comparing the results for }%DIFDELCMD < \asw{0h2m}%%%
\DIFdel{, shown in \figref{kapenc_compare_faulty}: 
...
\DIFdelbegin \DIFdel{Comparing the models from volunteers and experts can be done in \figref{kapenc_compare_faulty}, where only the
expert got the right configuration, but all the resulting models are comparable, besides rotation.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Two additional sims were modeled by an expert, \asw{1hpf} and \asw{0vqg}.
...
First let us recall \DIFdelbegin \DIFdel{the formulasfor comoving distances. }\begin{displaymath}\DIFdel{
D = \int \frac{dz}{\sqrt{\Omega_m(1+z)^3 + \Omega_\Lambda}}
}\end{displaymath}
%DIFAUXCMD
\DIFdel{The comoving distances
}\DIFdelend \DIFaddbegin \DIFadd{various distance formulas. Comoving distances
involve integrals of the type
}\begin{equation}\DIFadd{
\int \frac{dz}{\sqrt{\Omega_m(1+z)^3 + \Omega_\Lambda}}
%DIF > \raise.5ex\hbox{.}
}\end{equation}
\DIFadd{Specifically, the comoving distance $D_S$ }\DIFaddend from observer to \DIFdelbegin \DIFdel{lens,
}\DIFdelend \DIFaddbegin \DIFadd{source,
$D_{LS}$ }\DIFaddend from lens to source, and \DIFaddbegin \DIFadd{$D_L$ }\DIFaddend from observer to \DIFdelbegin \DIFdel{source are
given as follows:
}\DIFdelend \DIFaddbegin \DIFadd{lens are
...
$D$ by \DIFdelbegin \DIFdel{:
}\DIFdelend \DIFaddbegin \DIFadd{scale factors, as follows.
}\DIFaddend \begin{equation}
\begin{aligned}
D_S &= (1+z_S) \, d_S \\
...
\end{aligned}
\end{equation}
\DIFdelbegin \DIFdel{Angular-diameter }\DIFdelend \DIFaddbegin \DIFadd{Consequently, angular-diameter }\DIFaddend distances are always smaller than
comoving distances.  \DIFaddbegin \DIFadd{Moreover, while comoving distances can be simply
...
\section{\DIFadd{Miscellaneous}}

\DIFadd{Maybe discard \figref{ER_all_models}, since the essential information is there in \figref{ER_per_sim}.
}

\begin{figure}[htbp]
  \centering
    \includegraphics[width=0.80\textwidth]{fig/eR_1.png}
  \caption{\ERf \DIFaddFL{for all models with estimated errors in blue squares, }\ERg \DIFaddFL{of simulation in red crosses}}
