...
(EB, CC, CM, JO, PS and JW) while modelling was in progress.  After
the modelling stage, AM released the details of the sims for
post-modelling analysis.  Results from the latter now follow.

\subsection{Some example models} \label{sec:example_models}

Of the 119 models proposed, we now present eight examples in some
...
200 models generated by \spl.

We should remark that the scales and alignments in the six panels are
not completely consistent: the three panels on the right exactly the
same coordinates.  The middle-left and lower-left panels are
consistent with each other, but only approximately aligned with the
right column, becuse the latter depends on inexact mouse placements.
The upper-left panel is zoomed out slightly, to accommodate all the
markup.  For qualitative features, which is all that
Figures~\ref{fig:6941}--\ref{fig:7022} try to convey, these small
inconsistencies are harmless.
...
The model synthetic images is very crude, because there is no fitting
for the source light distribution, as already noted in \secref{diag}.
...
appearance of the arcs looks like an arc and counter-image such as
